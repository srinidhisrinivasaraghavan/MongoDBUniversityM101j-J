...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
        ...
        Document userSignUpDoc = new Document();
        userSignUpDoc.append("_id", username).append("password", passwordHash);

        if (email != null && !email.equals("")) {
            //  WORK HERE
            // if there is an email address specified, add it to the document too.
        	
        	userSignUpDoc.append("email", email);
        }

        try {
            //  WORK HERE
            // insert the document into the user collection here
        	usersCollection.insertOne(userSignUpDoc);
            return true;
